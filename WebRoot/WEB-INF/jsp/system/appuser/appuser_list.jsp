...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="appuser/appUserList.do" method="post" name="userForm" id="userForm">
			<table border='0'>
				<tr>
					<td>
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<!-- 开始循环 -->	
				<c:choose>
					<c:when test="${not empty appUserList}">
						<c:forEach items="${appUserList}" var="user" varStatus="vs">
							...
							<tr>
								...
								<c:choose>
									<c:when test="${empty user.RESIDENCE_NAME or user.RESIDENCE_NAME==''}">
										<td style="color: red;font-size: 12px;">未完善资料</td>
									</c:when>
									<c:otherwise>
										<td style="color: blue;font-size: 12px;">已完善资料</td>
									</c:otherwise>
								</c:choose>
								<td>${user.PHONE }</td>
								...
								<td style="width: 60px;" class="center">
									<c:if test="${user.STATUS == '0' }"><span class="label label-important arrowed-in">已停用</span></c:if>
									<c:if test="${user.STATUS == '1' }"><span class="label label-success arrowed">使用中</span></c:if>
								</td>
								<td>${user.occupation}</td>
								<td>${user.estate_company_name}</td>
								<td>${user.RESIDENCE_NAME}</td>
